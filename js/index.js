...
let dataMass = [...DATA];
const loadWrap = document.querySelector(".trainers-cards");
const wrap = document.querySelector(".page-wrapper");
const sorting = document.querySelector(".sorting");
const sidebar = document.querySelector(".sidebar");
const sortingBtn = document.querySelectorAll(".sorting__btn");
const tabs = document.querySelector(".sorting");
const tabsEl = document.querySelectorAll(".sorting__element");
const form = document.querySelector(".filters");
const asideFilter = document.querySelectorAll(".filters");
...
// *===================================================================================================
// !=======================================картки============================================================
function createCart(trainer) {
  const template = document
    .querySelector("#trainer-card")
    .content.cloneNode(true);

  const trainerInfo = template.querySelector(".trainer");
  trainerInfo.querySelector(".trainer__img").src = trainer.photo;
  trainerInfo.querySelector(
    ".trainer__img"
  ...
  trainersCardsContainer.appendChild(trainerInfo);
}

function outputTrainer(trainer) {
  trainersCardsContainer.innerHTML = "";
  trainer.forEach(createCart);
}
outputTrainer(dataMass);
// !======================================модалка=============================================================
function showModal(trainer) {
  ...
  modalBody.querySelector(".modal__img").src = trainer.photo;
  ...
  modalBody.querySelector(".modal__text").textContent = trainer.description;
  modalBody.querySelector(
...
tabs.addEventListener("click", (elem) => {
  if (elem.target.classList.contains("sorting__btn")) {
    sortingBtn.forEach((tab) => {
      tab.classList.remove("sorting__btn--active");
    });
    elem.target.classList.add("sorting__btn--active");
  }
});
sortingBtn.forEach((button) => {
  button.addEventListener("click", () => {
    const sortedTrainers = [...dataMass];
...
form.addEventListener("submit", function (event) {
  ...
  const selectedCategoryValue =
    form
      ...
      ?.value.trim()
      .toLowerCase() || "all";
  const selectedDirection = translationText[selectedDirectionValue] || "all";
  const selectedCategory = translationCategory[selectedCategoryValue] || "all";
  console.log(selectedDirection);
  console.log(selectedCategory);
  dataMass = DATA;
  if (selectedDirection !== "all") {
    dataMass = dataMass.filter((trainer) => {
      const trainerSpecialization =
        trainer.specialization.toLowerCase().trim() || "";
      return trainerSpecialization === selectedDirection.toLowerCase();
    });
  }
  if (selectedCategory !== "all") {
    dataMass = dataMass.filter((trainer) => {
      const trainerCategory = trainer.category.toLowerCase().trim() || "";
      return trainerCategory === selectedCategory.toLowerCase();
    });
  }
  outputTrainer(dataMass);
});
